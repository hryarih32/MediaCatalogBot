...
async def display_radarr_queue_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, page: int = 1) -> None:
    ...
    if user_role != app_config_holder.ROLE_ADMIN:
        logger.warning(
            f"Radarr queue attempt by non-admin {chat_id} (Role: {user_role}).")
    ...
    if not app_config_holder.is_radarr_enabled():
        logger.info(
            f"Radarr queue menu request by {chat_id}, but Radarr feature is disabled.")
    ...
    queue_data = get_radarr_queue(page=page, page_size=items_per_page)
    keyboard = []
